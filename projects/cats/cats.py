...
def about(subject):
    ...
    def about_select(sentence):
        re_sen = remove_punctuation(sentence)
        lo_re_sen = lower(re_sen)
        sp_lo_re_sen = split(lo_re_sen)
        for word in subject:
            if word in sp_lo_re_sen:
                return True
        return False
    return about_select        
...
def report_progress(typed, source, user_id, upload):
    ...
    # BEGIN PROBLEM 8
    cnt = 0
    for word1, word2 in zip(typed, source):
        if word1 == word2:
            cnt += 1
        else:
            break
...
def time_per_word(words, timestamps_per_player):
    ...
    for player_timestamps in tpp:
        ...
        for i in range(len(player_timestamps) - 1):
            diff = player_timestamps[i+1] - player_timestamps[i]
            player_times.append(diff)
...
def fastest_words(words_and_times):
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    fastest_plyer = [[] for _ in player_indices]
    for word_index in word_indices:
        word_times = [times[player_index][word_index] for player_index in player_indices]
        fastest_plyer_index = word_times.index(min(word_times))
        fastest_plyer[fastest_plyer_index].append(words[word_index])
    return fastest_plyer
    # END PROBLEM 10
