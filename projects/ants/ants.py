...
class Ant(Insect):
    ...
    def double(self):
        ...
        # BEGIN Problem 12
        "*** YOUR CODE HERE ***"
        if hasattr(self, 'damage_doubled') and not getattr(self, 'damage_doubled', False):
          self.damage *= 2
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while place is not None and not place.is_hive:
            if self.lower_bound <= distance and distance <= self.upper_bound:
                if place.bees:
...
class HungryAnt(Ant):
    ...
    implemented = True
    food_cost = 4
    chew_cooldown = 3 # class attributes
